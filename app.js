...
// --- NEW: Cache for external sensor readings ---
let EXTERNAL_READINGS_CACHE = [];
let liveReadingIndex = 0; // Index to cycle through the cached readings

/**
 * --- NEW: Fetches sensor data from the external API and stores it in the cache.
 */
  ...
  return entries.map(([sid, vals]) => {
    ...
    return `
      <div class="sensor">
        ...
        <div class="sensor-val">H₂S: <strong>${h2s}</strong>%</div>
      </div>
    `;
  }).join('');
}
...
async function onTankSelected(shipId, tankId) {
  ...
  // This part will now be handled by the polling loop with simulated data
  // try {
...
async function initShipPage() {
  ...
  // --- MODIFIED: Poll every 2s using the cached external data ---
  ...
  window.__shipPoll = setInterval(async () => {
    ...
    if (EXTERNAL_READINGS_CACHE.length === 0) {
      console.warn("External readings cache is empty. No live data to show.");
      return;
    }
    ...
    try {
      ...
      // Create a "live" data structure similar to what the backend would send
      const live = {
        sensors: { 'Simulated-Sensor': mappedValues },
        aggregates: { display: mappedValues }
      };
...
document.addEventListener('DOMContentLoaded', () => {
  // --- MODIFIED: Fetch external data on application start ---
